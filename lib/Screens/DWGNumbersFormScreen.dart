...
import '../services/api_service_dwg_numbers.dart';

class DWGNumbersFormScreen extends StatefulWidget {
  const DWGNumbersFormScreen({super.key});

  @override
  _DWGNumbersFormScreenState createState() => _DWGNumbersFormScreenState();
}

class _DWGNumbersFormScreenState extends State<DWGNumbersFormScreen> {
  final ApiServiceDWGNumbers apiService = ApiServiceDWGNumbers();
  DWGNumbers? currentRecord;
  DWGNumbers? originalRecord;
  bool isLoading = true;
  String errorMessage = '';
  bool hasUnsavedChanges = false;
  bool isNewRecord = false;
  List<DWGNumbers> searchResults = [];
  ...
  Future<void> _loadFirstRecord() async {
    ...
    try {
      ...
      print('Loaded first record: ${record.no}');
    } catch (e) {
      setState(() {
        isLoading = false;
  ...
  Future<void> _loadLastRecord() async {
    ...
    try {
      ...
      print('Loaded last record: ${record.no}');
    } catch (e) {
      setState(() {
        isLoading = false;
        errorMessage = 'Failed to load data: $e';
  ...
  Future<void> _loadNextRecord() async {
    ...
    if (currentRecord != null) {
      ...
      try {
        ...
        print('Loaded next record: ${record.no}');
      } catch (e) {
        setState(() {
          isLoading = false;
          errorMessage = 'Failed to load data: $e';
  ...
  Future<void> _loadPreviousRecord() async {
    ...
    if (currentRecord != null && currentRecord!.position > 1) {
      ...
      try {
        ...
        print('Loaded previous record: ${record.no}');
      } catch (e) {
        setState(() {
          isLoading = false;
          errorMessage = 'Failed to load data: $e';
  ...
  bool _hasDataInFields() {
    return prefixController.text.isNotEmpty ||
  ...
  void _submitRecord() async {
    ...
    if (noController.text.isEmpty || descController.text.isEmpty) {
      setState(() {
        errorMessage = 'NO and DESC are required fields.';
      });
      return;
    }
    setState(() {
  ...
  void _submitChanges() async {
    ...
    try {
      final updatedRecord = DWGNumbers(
        prefix: prefixController.text,
        no: int.parse(noController.text),
        desc: descController.text,
        model: modelController.text,
  ...
  void _searchRecord() async {
    ...
    try {
      ...
      setState(() {
        isLoading = false;
        errorMessage = 'Failed to search records: $e';
      });
      print('Error: $e');
    }
  }
